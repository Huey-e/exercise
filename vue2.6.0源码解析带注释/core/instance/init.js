...
export function initMixin (Vue: Class<Component>) {
  ...
  Vue.prototype._init = function (options?: Object) {
    ...
    // a uid 每一个类型的实例 都会有一个 唯一标识
    vm._uid = uid++

    // a flag to avoid this being observed  标记 避免响应式
    vm._isVue = true

    // merge options  { el, data(), methods .... }  new Vue
    if (options && options._isComponent) { // 询问是否是组件
      ...
      // mergeOptions 可以简单的理解为合并, 为我们的 options 增加属性
    ...
    initState(vm)                     // 重点, 初始化状态数据 ( data, props 等 )
