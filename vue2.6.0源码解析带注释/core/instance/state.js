...
function initProps (vm: Component, propsOptions: Object) {
  ...
  for (const key in propsOptions) {
    ...
    // instantiation here.
    if (!(key in vm)) {
      proxy(vm, `_props`, key) // 将 _props 上的成员映射到 Vue 实例上, 使得不需要 app._props.xxx 来方法, 直接使用 app.xxx 来方法
    }
  }
...
function initData (vm: Component) {
  ...
  data = vm._data = typeof data === 'function' // 判断是不是函数
  ...
  while (i--) {
    ...
    if (props && hasOwn(props, key)) {
      ...
      proxy(vm, `_data`, key) // 循环了 data 的所有属性, 映射到 Vue 实例上, 
                                                // 就不需要使用 app._data.xxx 来访问
                                                // 而是使用 app.xxx 来方法
    }
  }
  
  // observe data  文件路径/core/observer
  observe(data, true /* asRootData */) // 在这里进行响应式化
}
...
export function defineComputed (
  ...
  const shouldCache = !isServerRendering()  // 是不是服务端渲染
