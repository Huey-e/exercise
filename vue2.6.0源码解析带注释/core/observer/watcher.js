...
let uid = 0 // 标识watcher 保证watcher唯一
...
export default class Watcher {
  ...
  lazy: boolean; // 计算属性（computed）, 和 watch 来控制不要让 Watcher 立即执行
  ...
  active: boolean;
                        ...
                        // 执行完成后会清空 watcher，等到下次访问再依赖收集，派发、更新
  deps: Array<Dep>;
  depIds: SimpleSet; // 保证数据唯一 避免重复渲染
  ...
  constructor (
    ...
    if (typeof expOrFn === 'function') { //如果是函数， 就是 render 函数
      this.getter = expOrFn
    } else {
   ...
   */
  get () {
    pushTarget(this) // 当前watcher放到全局作用域上
    let value
    const vm = this.vm
    try {
  ...
  cleanupDeps () {
    ...
    while (i--) {
      ...
      if (!this.newDepIds.has(dep.id)) { // 在 二次提交中 归档就是 让 旧的 deps（每一个成员） 和 新 的 newDeps 一致
        dep.removeSub(this)
      }
    }
    let tmp = this.depIds
